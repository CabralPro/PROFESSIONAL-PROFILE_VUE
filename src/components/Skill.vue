<template>
  <div class="skill" @mouseenter="percentBar = percent" @mouseleave="percentBar = 0">
    <h3 class="titlee" v-html="title"></h3>
    <div class="bar">
      <div class="filledbar" :style="`width: ${percentBar}%`"></div>
    </div>
    <div class="circle">
      <img v-if="image" class="iconify" :src="require(`@/assets/img/icons/${image}`)" :style="
          `font-size: ${size ? size : ''}pt;
          top:${top ? top + '%' : ''};
          left: ${left ? left + '%' : ''};margin:auto`
        " />

      <span v-else class="iconify" :data-icon="icon" :style="
          `font-size: ${size ? size : ''}pt;
          top:${top ? top + '%' : ''};
          color: ${color ? color : ''}`
        "></span>
